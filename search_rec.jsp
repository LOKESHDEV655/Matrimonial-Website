...
<%
    ...
    if(email != null && session.getAttribute(email) != null){
        try{
            ...
            if(request.getParameter("gender").trim().length()==0 || request.getParameter("mstatus").trim().length()==0 || request.getParameter("state").trim().length()==0 || request.getParameter("religion").trim().length()==0){
                ...
                String ms="AND (";
		String pt="";
		String mstatus[] = request.getParameterValues("mstatus");
		if (mstatus != null && mstatus.length != 0) {
                    for (int i = 0; i < mstatus.length; i++) {
			ms=ms+pt+"mstatus='"+mstatus[i]+"'";
			pt=" OR ";
                    }
		}
		ms=ms+") AND";                
...
<div class="grid_3">
  ...
  <div class="col-md-9 profile_left1">
    ...
    <%   
        ResultSet rs = st.executeQuery("select * from jeevansathi where gender='"+gender+"' "+ms+" state='"+state+"' AND religion='"+religion+"' AND email<>'"+email+"'");
        int flag=0;
        
        while(rs.next()){
            flag=1;
    %>   
    <div class="profile_top"><a href="view_profile.jsp?code=<%=rs.getString("code")%>">
      <h2><%=rs.getString("code")%></h2>
	    <div class="col-sm-3 profile_left-top">
	    	<img src="photo/<%=rs.getString("code")%>.jpg" class="img-responsive" alt=""/>
	    </div>
	    <div class="col-sm-3">
	    	...
	    	<table class="table_working_hours">
				...
				<tr class="opened_1">
                                ...
                                <tr class="opened_1">
                                    ...
                                    <td class="day_value"><%=rs.getString("height")%></td>
				</tr>
				<tr class="opened_1">
                                    <td class="day_label1">City :</td>
                                    <td class="day_value"><%=rs.getString("city")%></td>
				</tr>
			    </tbody>
		   </table>
		   <div class="buttons">
